...
set OBOFile [lindex $argv 0]
set OUTFile [lindex $argv 1]
if {$OUTFile == ""} {set OUTFile "${OBOFile}.tab"}
::File::Open  $OUTFile 100 "no"
::OBO::Load   $OBOFile
foreach OBOID [::OBO::Query $OBOFile Term] {
    set OBOType [join [::OBO::Query   $OBOFile Term $OBOID namespace     ] "|"]
    set OBODef  [::OBO::Query   $OBOFile Term $OBOID name     ]
    set OBOSyn  [::OBO::Query   $OBOFile Term $OBOID Synonym  ]
    set OBOPar  [join [::OBO::Query   $OBOFile Term $OBOID Parents       ] "|"]
    set OBOPars [join [::OBO::Parents $OBOFile      $OBOID 0             ] "|"]

    set OBOSyn  [join [ListOfNonRedundantElement [concat $OBODef $OBOSyn]] "|"]
    set OBODef  [join $OBODef                                              "|"]

    regsub -all {\"| \[[^\]]*\]} $OBOSyn "" OBOSyn
    set NewLine [join [list $OBOID $OBOType $OBODef $OBOSyn $OBOPar $OBOPars] "\t"]
    ::File::Save  $OUTFile $NewLine
}
::File::Close $OUTFile
::OBO::Delete $OBOFile
