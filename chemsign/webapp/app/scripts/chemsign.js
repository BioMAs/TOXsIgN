...
angular.module('chemsign').controller('appCtrl',
    function ($scope,$rootScope, $log, Auth, User,$cookieStore, $location,Dataset) {
        ...
        if(user !== null && user !== undefined) {
            ...
            //console.log(cookie_selectID);
            //console.log(cookie_jobID);
           ...
           if (cookie_jobID != undefined && cookie_jobID != ''){
                var user_jobID = user.jobID.split(',');
                var cookies_listjob = cookie_jobID.split(',');
                //console.log(cookies_listjob);


                for(var i=0;i < cookies_listjob.length; i++){
                    var index = user_jobID.indexOf(cookies_listjob[i]);
                    //console.log(index);
                    if (index == -1){
                        user_jobID.push(cookies_listjob[i]);
                    }
                }
                user.jobID = user_jobID.join(',');
...
angular.module('chemsign').controller('queryCtrl',
    function ($scope,$rootScope, Auth, User, Dataset, Search, $filter, ngTableParams, $location,$window, $timeout) {
        $scope.selected_type = '';
        ...
        $scope.selected_field.signatures='';
            ...
            Search.search_index({"query":query_piece, 'query_dico':$scope.query,'number_query':number_query, 'pfrom':$scope.pfrom, 'sfrom':$scope.sfrom, 'sgfrom':$scope.sgfrom}).$promise.then(function(data){
              ...
              // }
                  ...
                  if (query_piece.includes('projects') && data['number'] != 0){
                    ...
                    $scope.signatures_number= data['number'];
                    $scope.projects_number = 'No Result';
                    $scope.studies_number = 'No Result';
                    $scope.assays_number = 'No Result';
                  }
                  ...
                  if (data['signatures'] != null){
                    $scope.search_signatures = data['signatures'].hits.hits;
                    for(var i =0;i<$scope.search_signatures.length;i++){
                      $scope.signatures.push($scope.search_signatures[i]['_source']);
                    }
                  }
                  else{
                    $scope.signatures_number="No Result";
...
angular.module('chemsign').controller('searchCtrl',
    function($scope, $rootScope, $routeParams, $log, $location, $window, User, Auth, SearchHits, Search) {
        var data = SearchHits.getHits();
        $scope.projects = [];
        $scope.studies = [];
        $scope.signatures = [];
        $scope.results="true";
        ...
        $scope.more = function(type){
          ...
          console.log($scope.sgfrom);
            ...
            Search.search_index({"query":query, 'search':'true','pfrom':$scope.pfrom, 'sfrom':$scope.sfrom, 'sgfrom':$scope.sgfrom}).$promise.then(function(data){
              ...
              console.log($scope.signatures_number);
              console.log($scope.sgfrom);
...
angular.module('chemsign').controller('jobsCtrl',
    function ($scope,$rootScope, $log, Auth, User,$window, $cookieStore, Dataset, $location, ngDialog, $route) {
...
angular.module('chemsign').controller('userCtrl',
  function($scope, $rootScope, $routeParams, $log, $location, $window, User,$route, Auth, Search, SearchHits) {
  ...
  function($scope, $rootScope, $routeParams, $log,$cookies, $cookieStore, $location, $window, Dataset, User, Upload, Auth, ngDialog) {
      ...
      $scope.addToWorkspace = function(id){
        ...
        if ($scope.user != null && $scope.user != undefined ){
            ...
            if ($cookieStore.get('selectedID') != undefined ){
                var cookie_selectID =  $cookieStore.get('selectedID').split(',');
            }
            else{
                var cookie_selectID = [];
            }
            ...
            var cookie = cookie_selectID.join(',');
          $cookieStore.put('selectedID', cookie);
        }
      }
      ...
      $scope.upExcel = function (obj,pid){
        console.log(obj);
        ngDialog.open({ template: 'saving', className: 'ngdialog-theme-default'})
        User.modifup({'uid': $scope.user.id, 'file': obj, 'pid' : pid}).$promise.then(function(data){
                alert(data.msg);
                ngDialog.close();
        });
      }
