...
source /home/genouest/irset/archives/web/TOXsIgN/sys/script/config.ini
...
function createRdata {
	logout=$LOG_PATH"TXS_RDATA_.out"
	RDATA=$ANNOTATION_PATH"signature_matrix.RData"
	qsub -N "TXS_RDATA" -j y -o $logout $SCRIPT_DIR"qsub_Rdata.sh" $SCRIPT_DIR"create_signmatrix4distance.R" $SIGNATURE_PATH "tmp_matrix" $RDATA
	while [ $(qstat | grep "TXS_RDAT" | wc -l) -ne 0 ]
	do
...
for i in $(find $SIGNATURE_PATH -name "[^.]*.sign"  -newer $SCRIPT_DIR"dateref")
	do
	echo $i
	OUTPUT_FILE=$i".enr"
	FINAL_FILE=$i".enr"
	...
	logoutSign=$LOG_PATH"TXS_signEnrich_"$x".out"
	...
	qsub -N "TXS_signEnrich_$x" -j y -o $logoutSign $SCRIPT_DIR""" $i $RDATA $SCRIPT_DIR
	let "x = $x + 1"
done
...
while [ $(qstat | grep "TXS_sign" | wc -l) -ne 0 ]
	do
		echo "Running TXS_signEnrich"
		sleep 7
done
