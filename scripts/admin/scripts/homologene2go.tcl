...
for {set i 0} {$i < [llength $argv]} {incr i} {
    ...
    if {$What == "-h"} {
	puts "homologene2go.tcl, v. 1.0"
        puts "Command line:\nwish homologene2go.tcl -hg \[homologene.data file\] -gene2go \[gene2go file\] -goobo \[gene_ontology.obo file\] -o \[output directory\]"
        exit
    }
    if {$What == "-hg"} {
	incr i
    ...
    if {$What == "-gene2go"} {
	incr i
	set Gene2GOFile [lindex $argv $i]
    } 
    if {$What == "-goobo"} {
	incr i
	set GOOBOFile [lindex $argv $i]
    } 
     if {$What == "-o"} {
	incr i
...
if {$Gene2GOFile == "" || ![file exists $Gene2GOFile]} {
    puts stderr "The gene2go file does not exist: $Gene2GOFile"
    exit
}
if {$GOOBOFile == "" || ![file exists $GOOBOFile]} {
    puts stderr "The gene_ontology.oboo file does not exist: $GOOBOFile"
    exit
}
...
#----------------------------------#
set ListOfGOTypes [list Process Function Component]
set ListOfHGIDs   {}
...
while {[gets $F Line]>=0} {
    ...
    foreach GOType $ListOfGOTypes {
	set TabHGID2GOID($HGID,$GOType)     {}
    }
...
#----------------------------------#
puts "$Gene2GOFile is loading..."
set F [open $Gene2GOFile]
while {[gets $F Line]>=0} {
    set GENEID ""
    foreach {TAXID GENEID GOID EVI NA TERM NA GOTYPE} [lrange [split $Line "\t"] 0 7] {}
    if {$GENEID == ""} {continue}
    if {![info exists TabGeneID2HGID($GENEID)]} {continue}
    set HGID $TabGeneID2HGID($GENEID)
    lappend TabHGID2GOID($HGID,$GOTYPE) $GOID
}
close $F
puts "$Gene2GOFile loaded!"
#----------------------------------#

#----------------------------------#
set OutFile "$OutDir/homologene2go"
::OBO::Load   $GOOBOFile
puts "$OutFile is saving..."
::File::Open  $OutFile 1000 "no"
foreach GOType $ListOfGOTypes {
    foreach HGID [ListOfNonRedundantElement $ListOfHGIDs] {
	set GOIDs [ListOfNonRedundantElement $TabHGID2GOID($HGID,$GOType)]
	foreach GOID [::OBO::Parents $GOOBOFile $GOIDs] {
	    set GOTerm [lindex [::OBO::Query $GOOBOFile Term $GOID name] 0]
	    if {$GOTerm == ""} {continue}
	    ::File::Save $OutFile "$HGID\t$GOType\t$GOTerm ($GOID)"
	}
    }
}
::File::Close $OutFile
puts "$OutFile saved!"
::OBO::Delete $GOOBOFile
#----------------------------------#
